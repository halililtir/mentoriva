...
const LIMITS = { respond: { max: 5, window: 60 }, chat: { max: 15, window: 60 } };
const DAILY_MAX = 50;

export async function checkRateLimit(ip: string, scope: RateLimitScope, username?: string): Promise<RateLimitResult> {
  // Admin kullanıcıları rate limit'ten muaf
  if (username === 'admin' || username === 'info@mentoriva.com.tr') {
